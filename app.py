...
def resetar_variaveis():
    ...
    img_removidas = 0
    df = pd.DataFrame(
        columns=["nome_imagem", "classe_predicao", "porcentagem_predicao"]
    )
    filenames = []
...
def atualiza_predicao_dataset(lamina):
    ...
    if not predicao_existente == "Positiva":
        ...
        if existe_classe_positiva:
            ...
            df_laminas.loc[df_laminas["nome_lamina"] == lamina, "classe_predicao"] = (
                "Positiva"
            )
        else:
        ...
        # alterando a data
        df_laminas.loc[df_laminas["nome_lamina"] == lamina, "data_atualizacao"] = (
            datetime.now()
        )
...
def verifica_imagem_upload():
    ...
    for filename in imagens:
        ...
        features = np.reshape(
            features, (1, -1)
        )  # Redimensiona as características para que tenham a mesma forma usada durante o treinamento
